    ...
    Promise.all(priorityPromises).then(results => {
        ...
        return Promise.all(displayPromises).then(displayResults => {
            ...
            if (displayResults[2].Priority) {
                //UoM for Months requires a different approach
                if (displayResults[2].FinalDueDateUoM === 'MON'){
                    //calculate milisecond difference
                    let today = new Date(), dueDate = new Date();
                    dueDate.setMonth(dueDate.getMonth() + parseInt(displayResults[2].FinalDueDateDuration));
                    let mSeconds = (dueDate.getTime() - today.getTime());
                    //update dueDate with difference
                    dueDate = new Date(new Date().getTime() + mSeconds);
