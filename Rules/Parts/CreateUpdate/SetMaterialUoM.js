...
export default function SetMaterialUoM(context) {
    ...
    try {
        ...
        let batchListPicker = !isFieldServiceTechnician && context.getPageProxy().evaluateTargetPathForAPI('#Control:BatchNumberLstPkr');
        ...
        if (context.getValue().length > 0) {
            ...
            if (isFieldServiceTechnician && (pageName === 'VehiclePartCreate' || pageName === 'VehicleIssueOrReceiptCreatePage')) {
            ...
            context.read(service, returnValue, [], query).then(result => {
                if (result && result.getItem(0)) {
                    ...
                    if (batchListPicker && !isFieldServiceTechnician) {
        ...
        // if (context.getPageProxy().evaluateTargetPathForAPI('#Control:OnlineSwitch').getValue()) {
            //     //materialUOMLstPkrSpecifier.setService('/SAPAssetManager/Services/OnlineAssetManager.service');
            // }
        if (!isFieldServiceTechnician && context.binding) {
